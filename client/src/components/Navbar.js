    ...
    <nav className="sticky top-0 z-50 border-b border-white/20 bg-gradient-to-r from-purple-950/90 via-purple-900/90 to-purple-950/90 px-4 py-3 text-white shadow-[0_16px_45px_rgba(20,8,35,0.5)] backdrop-blur-2xl supports-[backdrop-filter]:bg-purple-950/80 md:px-8">
      <div className="mx-auto flex max-w-7xl items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          />
          <span className="bg-gradient-to-r from-yellow-200 via-yellow-400 to-white bg-clip-text text-base font-semibold tracking-[0.14em] text-transparent md:text-lg">
            S Healing Center
          </span>
        </div>
        ...
        <div className="hidden items-center space-x-7 text-[0.92rem] tracking-[0.08em] md:flex">
          {[
            ...
            <Link key={to} to={to} className="relative transition duration-300 hover:text-yellow-300 hover:drop-shadow-[0_0_12px_rgba(212,175,55,0.4)] after:absolute after:-bottom-1 after:left-0 after:h-px after:w-0 after:bg-yellow-300 after:transition-all after:duration-300 hover:after:w-full">
              {label}
            </Link>
          ))}
        </div>
        ...
        <div className="flex items-center space-x-3 md:space-x-4">
          <div className="relative" ref={dropdownRef}>
            <button onClick={() => setOpen(!open)} className="rounded-full p-1.5 transition duration-300 hover:bg-white/10 hover:shadow-[0_0_18px_rgba(255,255,255,0.2)]">
